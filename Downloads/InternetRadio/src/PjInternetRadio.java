...
protected void buildGUI()
{
   ...
   MyClickableShape[] zeev = new MyClickableShape[4];
   for (int i = 0; i < zeev.length; i++) {
      zeev[i] = new MyClickableShape();
      zeev[i].setWidth(200);
      zeev[i].setHeight(200);

   if (i == 0) {
      zeev[i].setLocation(100, 100);
   }

   else {
      zeev[i].setLocation(zeev[i-1].getX() + 300, 100);
   }
          // assign image paths to the shape     //

   zeev[i].setImagePaths(kRELATIVE_IMAGE_PATHS);
